      ...
      <div className="flex w-full flex-col justify-between pl-[0.375rem]">
        <p className="flex flex-row text-[0.75rem] font-semibold leading-[1.125rem] text-sc-2 dark:text-light">
          ...
          {loggedInFollowerFilter && (
            <span className="ml-[0.25rem] flex h-[1.25rem] w-[1.375rem] items-center justify-center rounded-sm bg-red-80 text-[0.563rem] font-semibold leading-[0.875rem] text-light">
              {peopleFollowed}
            </span>
          )}
        </p>
        <p className="line-clamp-1 hidden text-[0.5625rem] text-sc-3 lg:block">
          {description}
        </p>
      </div>
    </Link>
