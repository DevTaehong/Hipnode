...
const PodcastPlayer = () => {
  const { isPlaying, togglePlay, songUrl, setSongUrl } = usePodcastStore();
  ...
  return (
    <div
      className={`bg-light_dark-3 fixed bottom-0 flex h-20 w-full items-center justify-between px-5 transition duration-200 ${
        !showPlayer && "translate-y-20"
      }`}
    >
      ...
      <div className="flex w-80 flex-col items-center">
        <div className="flex gap-2">
          ...
          <div
            ...
            className="flex-center cursor-pointer pl-2"
          >
            {isPlaying ? (
        ...
        <div className="flex w-full max-w-[20rem] items-center gap-3">
          <p className="text-sc-1_light-2">{formattedTime}</p>
          <Progress
            value={percentagePlayed}
            onClick={(percentage) => handleProgressClick(percentage)}
          />
          <p className="text-sc-1_light-2">{formattedLength}</p>
        </div>
      </div>
